...
class VerifierResult(BaseModel):
    """One verifier model's results in a cross-model comparison."""

    verifier: str  # backend name, e.g. "qwen" | "gemini"
    is_generator: bool  # True if this model generated the real-subset captions
    n_scored: int
    best_confidence_threshold: float
    extraction_recall: float
    sweep: list[SweepPoint]
    metrics: list[MethodMetrics]  # per subset; method == verifier name
...
class CrossModelReport(BaseModel):
    ...
    "which VLM audits best" comparison. On the **real** subset the captions
    were generated by ``generator_model``; that verifier's results are a
    *self-audit* (a model checking its own output) and the others are
    *cross-audits* — the gap between them quantifies the self-consistency
    blind spot.
    """
    ...
    baseline: list[MethodMetrics]  # per subset; verifier-independent
...
def _score_vidaudit_at(
    samples: Sequence[EvalSample],
    auditor: Callable[[EvalSample, float], SegmentAuditResult | None],
    threshold: float,
) -> tuple[dict[str, ConfusionMatrix], int, float]:
    """Score vidaudit at one confidence threshold, split by subset.

    Returns ``(per_subset_confusion, n_scored, extraction_recall)``. Samples
    the auditor can't audit (``None``) are skipped.
    """
    cm = {"synthetic": ConfusionMatrix(), "real": ConfusionMatrix()}
    extracted_hits = 0
    extracted_total = 0
    n_scored = 0
    for sample in samples:
        audit = auditor(sample, threshold)
        if audit is None:
            continue
        outcome = classify_vidaudit(sample, audit)
        cm[_subset_of(sample)].add(outcome)
    ...
    return cm, n_scored, extraction_recall


def _score_baseline(
    samples: Sequence[EvalSample],
    caption_for: Callable[[EvalSample], str | None],
    similarity_threshold: float,
) -> dict[str, ConfusionMatrix]:
    """Score the text-comparison baseline, split by subset."""
    cm = {"synthetic": ConfusionMatrix(), "real": ConfusionMatrix()}
    for sample in samples:
        caption = caption_for(sample)
        if caption is None:
            continue
        cm[_subset_of(sample)].add(classify_baseline(sample, caption, similarity_threshold))
    return cm
...
def run_eval(
    ...
    *,
    ...
    vid_cm, n_scored, extraction_recall = _score_vidaudit_at(samples, auditor, best_ct)
    base_cm = _score_baseline(samples, baseline_caption_for, baseline_similarity_threshold)
...
def run_cross_model_eval(
    samples: Sequence[EvalSample],
    auditors: dict[str, Callable[[EvalSample, float], SegmentAuditResult | None]],
    baseline_caption_for: Callable[[EvalSample], str | None] | None = None,
    *,
    ...
    the backend that produced the real-subset captions, so its row on the real
    subset is the *self-audit* and the rest are *cross-audits*.

    Args:
        samples: Labeled eval samples (synthetic + real).
        auditors: ``{verifier_name: auditor}`` — one entry per model to compare.
        baseline_caption_for: Optional text-comparison baseline caption source.
            Use a captioner that is NOT the real-caption generator, or the
            baseline degenerates to comparing identical text on the real subset.
        generator_model: The verifier name whose model generated the real
            captions (drives the self/cross labeling). ``None`` to omit.
        confidence_grid: Confidence thresholds swept per verifier.
        baseline_similarity_threshold: Below this Jaccard, the baseline flags.
        fallback_confidence_threshold: Used if a sweep is empty.

    Returns:
        A :class:`CrossModelReport`.
    """
    verifiers: list[VerifierResult] = []
    for name, auditor in auditors.items():
        sweep = _sweep_confidence(samples, auditor, confidence_grid)
        best_ct = _best_threshold(sweep, fallback_confidence_threshold)
        cm, n_scored, extraction_recall = _score_vidaudit_at(samples, auditor, best_ct)
        verifiers.append(
            VerifierResult(
                verifier=name,
                is_generator=(generator_model is not None and name == generator_model),
                n_scored=n_scored,
                best_confidence_threshold=best_ct,
                extraction_recall=extraction_recall,
                sweep=sweep,
                metrics=[
                    MethodMetrics(method=name, subset="synthetic", confusion=cm["synthetic"]),
                    MethodMetrics(method=name, subset="real", confusion=cm["real"]),
                ],
            )
        )

    baseline: list[MethodMetrics] = []
    if baseline_caption_for is not None:
        base_cm = _score_baseline(samples, baseline_caption_for, baseline_similarity_threshold)
        baseline = [
            MethodMetrics(method="baseline", subset="synthetic", confusion=base_cm["synthetic"]),
            MethodMetrics(method="baseline", subset="real", confusion=base_cm["real"]),
...
def render_cross_model_report(report: CrossModelReport, console: object | None = None) -> None:
    """Pretty-print the cross-model comparison matrix."""
    from rich.console import Console
    from rich.table import Table
    ...
    con.rule("[bold]vidaudit cross-model eval")
    con.print(
        f"samples: [bold]{report.n_samples}[/]  "
        f"real-caption generator: [bold]{report.generator_model or '—'}[/]\n"
    )

    table = Table(
        title="Hallucination-audit performance by verifier and subset",
        title_style="bold",
    )
    table.add_column("Verifier", style="bold")
    table.add_column("Subset")
    ...
    def _row(label: str, m: MethodMetrics, *, role: str = "") -> None:
        c = m.confusion
        table.add_row(
            label,
            m.subset,
    ...
    for v in report.verifiers:
        for m in v.metrics:
            role = ""
            if m.subset == "real":
                role = "self-audit" if v.is_generator else "cross-audit"
            _row(f"[green]{v.verifier}[/]", m, role=role)
    for m in report.baseline:
        _row("[yellow]baseline[/]", m)
